...
//    logger.info(s"Browser: ${driver.getCapabilities.getBrowserName} ${driver.getCapabilities.getBrowserVersion}")
  ...
  private def headless(capabilities: MutableCapabilities): MutableCapabilities = {
    val headless     = sys.props.getOrElse("browser.headless", "false").toBoolean
    val browserName  = capabilities.getBrowserName

    if (headless) {
      browserName match {
        case "chrome"        => capabilities.asInstanceOf[ChromeOptions].addArguments("--headless=new")
        case "MicrosoftEdge" => capabilities.asInstanceOf[EdgeOptions].addArguments("--headless=new")
  ...
  private def securityAssessment(capabilities: MutableCapabilities): MutableCapabilities = {
    ...
    if (enabledLocal || enabledBuild) {
      ...
      logger.info("Security assessment: Enabled")
    }
